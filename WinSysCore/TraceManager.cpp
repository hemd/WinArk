...
bool TraceManager::Start(EventCallback cb) {
	...
	// {A17F6E93-D74C-4662-8AE8-1BDA6B22B0FE}
	static const GUID dummyGuid =
	{ 0xa17f6e93, 0xd74c, 0x4662, { 0x8a, 0xe8, 0x1b, 0xda, 0x6b, 0x22, 0xb0, 0xfe } };
...
bool TraceManager::SwapFilters(int i1, int i2) {
	if (i1 < 0 || i2 < 0 || i1 >= _filters.size() || i2 >= _filters.size() || i2 == i1)
		return false;
...
uint32_t TraceManager::GetFilteredEventCount() const {
...
const std::wstring& TraceManager::GetKernelEventName(EVENT_RECORD* rec) const {
	...
	if (::TdhGetEventInformation(rec, 0, nullptr, info, &size) == STATUS_SUCCESS) {
		const auto name = std::wstring((PCWSTR)((PBYTE)info + info->TaskNameOffset)) + L"/" +
			std::wstring((PCWSTR)((PBYTE)info + info->OpcodeNameOffset));
...
bool TraceManager::SetBackupFile(PCWSTR path) {
	if (path) {
		...
		if (!hFile)
			return false;
		_hMemMap.reset(::CreateFileMapping(hFile.get(), nullptr, PAGE_READWRITE | MEM_RESERVE,
			32, 0, nullptr));
		if (!_hMemMap)
			return false;
	}
