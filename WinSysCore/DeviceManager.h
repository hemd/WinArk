...
namespace WinSys {
	...
	public:
		static std::unique_ptr<DeviceManager> Create(const wchar_t* computerName = nullptr,
		...
		// device
		std::wstring GetDeviceRegistryPropertyString(const DeviceInfo& di, DeviceRegistryPropertyType type);
		std::vector<std::wstring> GetDeviceRegistryPropertyMultiString(const DeviceInfo& di, DeviceRegistryPropertyType type);
		template<typename T>
		...
		static std::vector<std::wstring> GetDeviceClassRegistryPropertyMultiString(const GUID* guid,
			DeviceClassRegistryPropertyType type);
	...
	inline T DeviceManager::GetDeviceRegistryProperty(const DeviceInfo& di, DeviceRegistryPropertyType type) const {
		// trivially_copyable 拷贝不变
		static_assert(std::is_trivially_copyable<T>());
		T result{};

		::SetupDiGetDeviceRegistryProperty(_hInfoSet.get(), (PSP_DEVINFO_DATA)&di.Data,
			static_cast<DWORD>(type), nullptr, (BYTE*)&result, sizeof(T), nullptr);
		return result;
	}

	template<typename T>
	inline T DeviceManager::GetDeviceClassRegistryProperty(const GUID* guid, DeviceClassRegistryPropertyType type) {
		static_assert(std::is_trivially_copyable<T>()); // 拷贝不变
		T result{};
		DWORD regType;
		::SetupDiGetClassRegistryProperty(guid, static_cast<DWORD>(type), &regType,
