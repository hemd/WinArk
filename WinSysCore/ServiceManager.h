...
namespace WinSys {
	...
	struct ServiceConfiguration {
		...
		std::wstring DispalyName;
		bool DeleayedAutoStart;
		bool TriggerStart;
	};
	...
	public:
		...
		static bool Uninstall(const std::wstring& nmae);

		static std::unique_ptr<Service> Install(const std::wstring& name, const std::wstring& displayName,
			ServiceAccessMask desiredAccess, ServiceType type, ServiceStartType startType,
			ServiceErrorControl errorControl, const std::wstring& imagePath);
		static std::unique_ptr<Service> Install(const std::wstring& name, ServiceType type,
			ServiceStartType startType, const std::wstring& imagePath);
