...
std::unique_ptr<ServiceConfiguration> ServiceManager::GetServiceConfiguration(const std::wstring& serviceName) {
	...
	result->DispalyName = config->lpDisplayName;
	DWORD len;
	if (result->AccountName.empty() && (config->dwServiceType & SERVICE_USER_SERVICE)) {
		// user service (Windows 10+)
		// extract LUID
		auto pos = result->DispalyName.rfind(L'_');
		if (pos != std::wstring::npos) {
			auto logonId = wcstoll(result->DispalyName.c_str() + pos + 1, nullptr, 16);
	...
	if (result->StartType == ServiceStartType::Auto) {
		...
		if (ok)
			...
			result->DeleayedAutoStart = false;
	}
...
wil::unique_schandle ServiceManager::OpenServiceHandle(const std::wstring& name, ServiceAccessMask access) {
	...
	wil::unique_schandle hService(::OpenService(hScm.get(), name.c_str(),
...
std::unique_ptr<WinSys::Service> ServiceManager::Install(
	...
	wil::unique_schandle hService(::CreateService(hScm.get(), name.c_str(),
...
std::unique_ptr<WinSys::Service> ServiceManager::Install(const std::wstring& name,
	ServiceType type, ServiceStartType startType,
	const std::wstring& imagePath) {
	return Install(name, L"", ServiceAccessMask::AllAccess, type, startType,
		ServiceErrorControl::Normal, imagePath);
}
...
Sid ServiceManager::GetServiceSid(const wchar_t* name) {
	...
	WCHAR domain[32];
	DWORD domainSize = _countof(domain), sidSize = _countof(sid);
	SID_NAME_USE use;
	...
	if (::LookupAccountName(nullptr, serviceName.c_str(), (PSID)sid,
		&sidSize, domain, &domainSize, &use))
		return Sid((PSID)sid);
