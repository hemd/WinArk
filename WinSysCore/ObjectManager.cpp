...
bool ObjectManager::EnumHandlesAndObjects(PCWSTR type, DWORD pid, PCWSTR prefix, bool namedOnly) {
	...
	std::wstring sprefix(prefix ? prefix : L"");
	...
	for (decltype(count) i = 0; i < count; i++) {
		...
		if (auto it = _objectsByAddress.find(handle.Object); it == _objectsByAddress.end()) {
			...
			obj->TypeName = GetType(obj->TypeIndex)->TypeName.c_str();
...
std::wstring ObjectManager::GetSymbolicLinkTarget(PCWSTR path) {
	...
	if (NT_SUCCESS(::NtOpenSymbolicLinkObject(hLink.addressof(), GENERIC_READ, &attr))) {
		...
		if (NT_SUCCESS(::NtQuerySymbolicLinkObject(hLink.get(), &result, nullptr)))
			target = std::wstring(result.Buffer, result.Length / sizeof(WCHAR));
	}
	return target;
}
...
std::wstring ObjectManager::GetObjectName(HANDLE hDup, USHORT type) {
