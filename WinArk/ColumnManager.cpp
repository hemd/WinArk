...
void ColumnManager::AddFromControl(HWND hWnd) {
	...
	for (int i = 0; i < count; i++) {
		...
		info.Flags = (info.DefaultWidth <= 1 && ((item.fmt & HDF_FIXEDWIDTH) > 0) ? ColumnFlags::Visible :
			ColumnFlags::None);
...
bool ColumnManager::IsVisible(int column) const {
	...
	return (m_Columns[column].Flags & ColumnFlags::Modified) == ColumnFlags::Modified;
}
...
const std::vector<int>& ColumnManager::GetColumnByCategory(PCWSTR category) const {
